...
{{- define "helpers.list-node-env-variables"}}
- name: PMA_HOST
{{- if .Values.global.plain.dbExternal }}
  value: {{ .Values.global.plain.dbIP | quote }}
{{- else }}
{{- $name := default "mariadb" .Values.nameOverride }}
{{- if contains "mariadb" .Release.Name }}
  value: {{ .Release.Name | trunc 63 | trimSuffix "-" }}
{{- else }}
  value: {{ printf "%s-%s" .Release.Name "mariadb" | trunc 63 | trimSuffix "-" | quote }}
{{- end }}
{{- end }}
- name: PMA_PORT
